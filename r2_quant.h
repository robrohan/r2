...
#define R2_QUANT_EPSILON 1e-8f
    ...
    /* ---------------- int16 ----------------
       NOTE: dot/matmul accumulators here are int64_t, not int32_t.
       32767*32767 ~= 1.07e9 - summing as few as ~2 max-magnitude products
       already overflows an int32_t accumulator. Do not "simplify" these
       back down to int32_t. */
    ...
    /* ---------------- int4 ----------------
       Packed 2 values/byte: element 2k lives in the low nibble of byte k,
       element 2k+1 in the high nibble. Range is -7..7 (not the full -8..7)
       to match int8/int16's symmetric-range convention. Kernels decode
       nibbles on the fly in their inner loop rather than unpacking a whole
       row/matrix into a buffer sized by n - that would cost back the memory
       int4 exists to save. */
    ...
    static void mat_mul_i4(const uint8_t *packed_m1, const uint8_t *packed_m2, unsigned r1, unsigned c1,
                            unsigned r2, unsigned c2, int32_t *out);
...
#include "r2_simd.h"
    ...
    static float r2_quantize_i8(const float *in, int n, int8_t *out)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            if (qi > R2_I8_QMAX) qi = R2_I8_QMAX;
            if (qi < R2_I8_QMIN) qi = R2_I8_QMIN;
            out[i] = (int8_t)qi;
        }
        return scale;
    }
    ...
    static int32_t vecn_dot_i8(const int8_t *v1, const int8_t *v2, int n)
    {
        ...
        for (; i < n; i++) sum += (int32_t)v1[i] * (int32_t)v2[i];
        return sum;
        ...
        int32_t sum = 0;
        int i;
        for (i = 0; i < n; i++) sum += (int32_t)v1[i] * (int32_t)v2[i];
        return sum;
#endif
    }
                            ...
                            int32_t *out)
    {
        ...
        for (i = 0; i < r1; i++)
        {
            ...
            for (j = 0; j < c2; j++) out_row[j] = 0;
            for (k = 0; k < c1; k++)
            {
                int32_t a = (int32_t)m1[i * c1 + k];
                const int8_t *R2_RESTRICT m2_row = m2 + k * c2;
                for (j = 0; j < c2; j++) out_row[j] += a * (int32_t)m2_row[j];
            }
        }
    }
                                    ...
                                    unsigned r2, unsigned c2, int8_t *out, float *out_scale)
    {
        ...
        /* No scratch buffer: the output scale isn't known until every
           element has been seen, so this recomputes the i-k-j accumulation
           twice (once to find max_abs, once to requantize) rather than
           allocating an r1*c2-sized int32 buffer to hold it in between. */
        float max_abs = 0.f;
        for (i = 0; i < r1; i++)
        {
            for (j = 0; j < c2; j++)
    ...
    /* ================================================================
     * int16 (int64 accumulator - see the overflow note in the declarations
     * above; 32767*32767 already needs 31 bits, so summing more than one
     * such product overflows int32_t)
     * ================================================================ */
    ...
    static float r2_quantize_i16(const float *in, int n, int16_t *out)
    {
        ...
        {
            int32_t qi = r2__round_i32(in[i] / scale);
            if (qi > R2_I16_QMAX) qi = R2_I16_QMAX;
            if (qi < R2_I16_QMIN) qi = R2_I16_QMIN;
            out[i] = (int16_t)qi;
        }
        return scale;
    }
    ...
    {
#if defined(R2_SIMD_AVX2)
        /* Each _mm256_madd_epi16 call pairwise-multiplies-and-adds 16 int16
           lanes into 8 int32 partial sums. Max magnitude of one such partial
           sum is 2*32767*32767 ~= 2.147e9, just under INT32_MAX - safe for a
           single madd result, but NOT safe to accumulate across chunks in
           int32. So each chunk's 8 int32 results are widened to int64 and
           folded into the running scalar accumulator immediately. */
        int64_t sum = 0;
        int i = 0;
        for (; i + 16 <= n; i += 16)
        {
            ...
            int k;
            for (k = 0; k < 8; k++) sum += (int64_t)buf[k];
        }
        for (; i < n; i++) sum += (int64_t)v1[i] * (int64_t)v2[i];
        return sum;
        ...
        for (; i + 8 <= n; i += 8)
        {
            ...
            int k;
            for (k = 0; k < 4; k++) sum += (int64_t)buf[k];
        }
        for (; i < n; i++) sum += (int64_t)v1[i] * (int64_t)v2[i];
        return sum;

#elif defined(R2_SIMD_NEON)
        /* Widen int16->int32 (vmull_s16, safe: max product ~1.07e9) then
           pairwise-widen-accumulate int32->int64 (vpadalq_s32) - the same
           "widen one level, then widen-accumulate" idiom as the int8 NEON
           kernel above, just one width class higher. */
        ...
        int64_t sum = 0;
        int i;
        for (i = 0; i < n; i++) sum += (int64_t)v1[i] * (int64_t)v2[i];
        return sum;
#endif
    }
    ...
    static void mat_mul_i16(const int16_t *m1, const int16_t *m2, unsigned r1, unsigned c1, unsigned r2,
                             unsigned c2, int64_t *out)
    {
        if (c1 != r2) return;
        ...
        for (i = 0; i < r1; i++)
        {
            ...
            for (k = 0; k < c1; k++)
            {
                ...
                for (j = 0; j < c2; j++) out_row[j] += a * (int64_t)m2_row[j];
            }
        }
    }
                               ...
                               unsigned r2, unsigned c2, float *out)
    {
        ...
        /* int64_t is twice the width of float, so out's storage can't be
           reused as scratch the way mat_mul_i8_f reuses an int32_t-sized
           buffer. Recomputed directly instead - no extra allocation. */
        float scale = s1 * s2;
        unsigned i, j, k;
        for (i = 0; i < r1; i++)
        {
            ...
            {
                int64_t acc = 0;
                for (k = 0; k < c1; k++) acc += (int64_t)m1[i * c1 + k] * (int64_t)m2[k * c2 + j];
                out[i * c2 + j] = (float)acc * scale;
            }
        }
    }
    ...
    {
        if (v > R2_I4_QMAX) v = R2_I4_QMAX;
        if (v < R2_I4_QMIN) v = R2_I4_QMIN;
        return (int8_t)v;
    }
    ...
    static void r2_pack_i4(const int8_t *in, int n, uint8_t *out)
    {
        ...
        if (n & 1) out[n >> 1] = r2__nibble_encode(r2__i4_clamp(in[n - 1]), 0);
    }
    ...
    static void r2_unpack_i4(const uint8_t *packed, int n, int8_t *out)
    {
        ...
        if (n & 1) out[n - 1] = r2__nibble_decode(packed[n >> 1], 0);
    }
    ...
    static void r2_dequantize_i4(const uint8_t *packed, int n, float scale, float *out)
    {
        ...
        if (n & 1) out[n - 1] = (float)r2__nibble_decode(packed[n >> 1], 0) * scale;
    }

    static int32_t vecn_dot_i4(const uint8_t *p1, const uint8_t *p2, int n)
    {
        /* Decode-on-the-fly, zero extra buffer: this is the whole point of
           int4 (memory savings), so we never unpack into a scratch array
           sized by n here - see the file header note and the mat_mul_i4
           comment below for the same policy applied to matmul. */
        int32_t sum = 0;
        int i;
        for (i = 0; i + 1 < n; i += 2)
        {
                            ...
                            unsigned r2, unsigned c2, int32_t *out)
    {
        ...
        /* Decodes both operands' nibbles inline, redundantly re-decoding
           packed_m2's row on every outer i iteration, rather than unpacking
           a row/matrix into an n-sized scratch buffer - trading some ALU
           cost for zero extra memory, which is the right trade for the
           embedded target this format exists for. */
        unsigned i, j, k;
        for (i = 0; i < r1; i++)
        {
            int32_t *R2_RESTRICT out_row = out + i * c2;
            ...
            for (k = 0; k < c1; k++)
            {
                ...
                for (j = 0; j < c2; j++) out_row[j] += a * (int32_t)r2__i4_at(packed_m2, k * c2 + j);
            }
        }
    }
